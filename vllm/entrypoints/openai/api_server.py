...
async def create_chat_completion(request: ChatCompletionRequest,
                                 raw_request: Request):
    model_name_b58 = raw_request.headers.get("x-model-name")
    if model_name_b58:
        try:
            import base58
            model_name = str(base58.b58decode(model_name_b58), "utf-8")
...
def generate_unique_int(exclude_list, start=0, end=100):
    while True:
        rand_int = random.randint(start, end)
        if rand_int not in exclude_list:
            return rand_int
